...
async function main() {
  ...
  console.log('Basado en tus capturas, ya tienes configurado Meta Developer.\n');

  // Instagram Configuration
  const instagramToken = await ask('1. Pega tu INSTAGRAM_ACCESS_TOKEN de Meta Developer: ');
  const instagramUserId = await ask('2. Confirma tu INSTAGRAM_ACCOUNT_ID (17841438252089801): ') || '17841438252089801';
  
  console.log('\n🌐 CONFIGURACIÓN DEL SITIO');
  const siteUrl = await ask('3. URL de tu sitio en Vercel (ej: https://reformas.vercel.app): ');

  console.log('\n📧 CONFIGURACIÓN DE EMAIL (OPCIONAL)');
  const emailUser = await ask('4. Tu email Gmail (opcional, presiona Enter para omitir): ');
  let emailPass = '';
  if (emailUser) {
  ...
  const supabaseUrl = await ask('6. URL de Supabase (opcional, presiona Enter para omitir): ');
  let supabaseAnonKey = '';
